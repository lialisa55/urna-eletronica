...
?>
<h1 class="row-3"> Seu voto para governador </h1>
<div class="container fluid">
  <div class="row">
    ...
    <div class='col-3 mt-5 mx-auto'>
      ...
      <p>10 - Tarcísio/Republicanos</p>
      <p>80 - Carol Vigliar/UP</p>
      <p>45 - Rodrigo Garcia/PSDB</p>
      <p>21 - Gabriel Golombo/PCB</p>
    </div>
  </div>
</div>
...
if(isset($_POST['codigo'])){
  if(empty($_POST['codigo'])){
    ...
    $print = mysqli_fetch_array($con->query("SELECT codigo, nome, partido, imagem from tb_governador WHERE codigo = $codigo;"));
...
if(isset($_POST['nome']) && isset($_POST['partido']) && isset($_POST['codigo'])){
    ...
    if(empty($_POST['nome']) || empty($_POST['partido']) || empty($_POST['codigo'])){
        ...
        $select = mysqli_fetch_array($con->query("SELECT votos from tb_governador where codigo = $codigo;"));
  
        $con->query("UPDATE tb_governador set votos = ". $select['votos'] + 1 ." WHERE codigo = $codigo;");
  
        echo('<script> window.location.replace("presidente.php");</script>');
